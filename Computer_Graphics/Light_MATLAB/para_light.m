...
% 参数
O = [0, 5, 0.5];           % 白色平行光源方向点，其中A^2+B^2+C^2 != 0
eye = [1, 1, 1];        % 观察点坐标
...
% 求取单位入射向量
point_incidence1 = par_incidence(X, Y, Z1, O);
...
color1 = ambient + diffuse * lamber1 + reflect * max(sum(eye1.*point_reflect1, 2), 0);
size_X = size(X);
...
% 求取单位入射向量
point_incidence2= par_incidence(X, Y2, Z2, O);
% 求单位法向量
...
color2 = ambient + diffuse * lamber2 + reflect * max(sum(eye2.*point_reflect2, 2), 0);
...
% 求取单位入射向量
point_incidence3= par_incidence(X3, Y, Z3, O);
% 求单位法向量
...
color3 = ambient + diffuse * lamber3 + reflect * max(sum(eye3.*point_reflect3, 2), 0);
