    ...
    for a=1:ceil(leng/8)
        for b=1:ceil(width/8)
            ...
            if(1<=block_num && block_num<=8)
                password_leng_bin=[password_leng_bin,char(block(para)+48)];
            ...
            if(block_num>=9 && block_num<=8*password_leng+8)
                ...
                password_bin=[password_bin,char(block(para)+48)];
                if(y==8)
                    password2=[password2,char(bin2dec(password_bin))];
                end
            end
            
            block(para)=0;%the most important step!!
            block=block.*QTAB;
            block=idct2(block);
            block=block+128;
