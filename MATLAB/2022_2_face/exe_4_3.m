...
block_leng=36; %1008=16*7*9 %not for all 
line_leng=6; %drew red line
freq=zeros(2^(3*L),1);
file=strcat('Faces\test.jpg');

pic=double(imrotate(imread(file,'jpg'),-90));
[leng,width,~]=size(pic);

% pic=imread(file,'jpg'); %横向拉伸
% [leng,width,~]=size(pic);
% pic=double(imresize(pic,[leng,width*2]));
% width=width*2;

% pic=imread(file,'jpg'); %改变颜色
% pic=imadjust(pic,[0.18 0.18 0.18;1 1 1],[0 0 0;1 1 1]);
% pic=double(pic);
% [leng,width,~]=size(pic);
    ...
    for b=1:(width/block_leng)
        ...
        for c=1:block_leng
            for d=1:block_leng
                ...
                G=pic_block(c,d,2);
                B=pic_block(c,d,3);
                num=fix(R/(2^(8-L)))*2^(2*L)+fix(G/(2^(8-L)))*2^L+fix(B/(2^(8-L)));%通过除法实现映射关系 %关于fix 存疑?
...
imshow(uint8(pic));
imwrite(uint8(pic),'exe_4_3_1.jpg','jpg');
%imwrite(uint8(pic),'exe_4_3_2.jpg','jpg');
%imwrite(uint8(pic),'exe_4_3_3.jpg','jpg');