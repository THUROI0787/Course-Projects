...
for a=1:ceil(leng/8)
    for b=1:ceil(width/8)
        ...
        if(1<=block_num && block_num<=7)
            [p,basic]=inv_remola(basic);
            password_leng_bin=[password_leng_bin,p];
        end
        if(block_num==8)
            [p,basic]=inv_remola(basic);
            password_leng_bin=[password_leng_bin,p];
        ...
        if(block_num>=9 && block_num<=8*password_leng+8)
            ...
            y=block_num-8*x;
            [p,block]=inv_remola(basic);
            if(y==1)
                password_bin=[];
            end
            password_bin=[password_bin,p];
            if(y==8)
                password=[password,char(bin2dec(password_bin))];
            end
        end
        re(a*8-7:a*8,b*8-7:b*8)=idct2((inv_zigzag(basic)).*QTAB)+128;
    end
end

%%
imshow(uint8(re));
imwrite(uint8(re),'exe_3_2_3.png','png');
password
load('hall.mat');
