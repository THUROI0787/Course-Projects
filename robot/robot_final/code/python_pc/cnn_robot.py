...
class myDataSet(data.Dataset):
    def __init__(self, is_test):
        ...
        for i in range(self.class_num):
            image_files = os.listdir('{}/class{}'.format(self.data_path, i + 1))
            file_count = len(image_files)

            if is_test:
                for j in range(test_samples):
                    image = Image.open(
                        '{}/class{}/{}.jpg'.format(self.data_path, i + 1, j + 1)).convert('RGB')
                    self.image.append(image)
                    self.label.append(i)
            else:
                train_samples = file_count - test_samples
                print("class: {}; training samples: {}".format(i+1, train_samples))
                for j in range(test_samples, file_count):
                    image = Image.open(
                        '{}/class{}/{}.jpg'.format(self.data_path, i + 1, j + 1)).convert('RGB')
                    self.image.append(image)
                    self.label.append(i)
