...
# vpath = 'STD_data/Test/Clean/vfeat'
# apath = 'STD_data/Test/Clean/afeat'
vpath = 'STD_data/Train&Test/Test_42/vfeat'
apath = 'STD_data/Train&Test/Test_42/afeat'

def gen_tsample(n):
    tsample = np.zeros((500, n)).astype(np.int16)
    for i in range(500):
        tsample[i] = np.random.permutation(804)[:n]
    np.save('save/tsample_{}.npy'.format(n), tsample)
...
np.save('save/rst_epoch{}.npy'.format(epoch), rst)
...
tsample = np.load('save/tsample_{}.npy'.format(50))
get_top(tsample, rst)
